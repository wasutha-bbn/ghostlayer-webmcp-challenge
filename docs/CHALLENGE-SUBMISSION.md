...
Replace every **[PENDING]** field before pasting this text into Devpost. This public package contains only the client-only challenge sandbox; the separate owner-only prototype is not included.
...
- [ ] Replace the live website placeholder with a functioning public judge URL.
- [ ] Confirm the URL opens without access being granted manually to each judge.
- [ ] Confirm `/pilot`, `/pilot/admin`, `/pilot/legacy`, and `/api/pilot/state` are absent from the challenge host.
- [ ] Replace the repository placeholder with a public GitHub, GitLab, or Bitbucket URL.
- [ ] Confirm `README.md`, setup commands, source, and `LICENSE` are visible in that public repository.
- [ ] Remove or sanitize private deployment identifiers and unrelated generated artifacts from the public package.
- [ ] Replace the video placeholder with a public YouTube URL.
- [ ] Confirm the video is shorter than three minutes and contains audible narration or audio.
